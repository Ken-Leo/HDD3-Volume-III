...
![](images/bbe88dc9f223b102c887d7279ce765e2786705e14c1bea91e89f91448d8b40c8.jpg)
...
```mermaid
graph TD
    A["Adjacent track"] --> B["MR read element"]
    C["Main track"] --> B
    D["Adjacent track"] --> B
    B --> E["Δ_T (Track offset)"]
    B --> F["T_z (Track pitch)"]
    B --> G["T_x (bit period)"]
...
รูปที่ 6.29 ลักษณะของแทรักษมิเสริจิสเตรชัน (TMR) ในระบบ BPMR

จากทร์เก็ด $^{40}$ ที่กำหนดไว้ เพาะฉะนั้นจะเห็นได้ว่างจรตรวจหา PRML ที่ใช้ในการลอตรหัสข้อมูลของระบบการบันทึกข้อมูลเชิงแม่เหล็กแบบแนวนอนและแบบแนวตั้ง [10] ยังคงสามารถนำมาใช้ลอตรหัสข้อมูลของระบบ BPMR ได้อย่างมีประสิทธิภาพเช่นกัน [108, 109]

# 6.4.4 แทร์กมิสเรจิสเตรชั้น

รูปที่ 6.7 แสดงตําแหน่งที่เหมาะสมตุขของหัวอ่านแบบ MR ที่อยู่บนสื่อบันทึก กล่าวคือจุดศูนย์กลาง ของหัวอ่านควรอยู่ ณ จุดกิ่งกลางของแทรกหลักและเคลื่อนที่ชนานไปกับแนวตามแทรก เพื่อให้สัญญาณอ่านกลับที่ต้านซออกของหัวอ่านมีคุณภาพดีสุด อย่างไรก็ตามในทางปฏิบัติหัวอ่านมีโอกาสเคลื่อนที่เลื่อนออกไปจากตําแหน่งที่ควรจะเป็น โดยล้าจุดศูนย์กลางของหัวอ่านจะเคลื่อนที่ไปในแนวขวางแทรกทางด้านบน (หรือด้านล่าง) ตามที่แสดงในรูปที่ 6.29 จะเรียกปรากฏการณ์นี้ว่า “แทรกมิสรเจิสเตอร์ชัน” (TMR: track mis-registration) หรือออฟเซ็ดของหัวอ่าน (read head offset) ซึ่งลูกกำหนดโดย [119]

$$
\mathrm{TMR} = \frac {\Delta_ {T}}{T _ {z}} \times 1 0 0 \tag {6.20}
$$

มีหน่วยเป็นเปอร์เซ็นต์ เมื่อ $\Delta_{T}$ (track offset) คือระยะที่หัวอ่านเคลื่อนที่ออกไปจากแทร็กหลัก ในแนวทางแทร็ก และ $T_{z}$ คือระยะแทร็ก (track pitch) ตั้งนั้นสัญญาณฟัลส์ BPMR แบบที่ไม่ต่อเนื่องทางเวลาที่มีผลกระทบของแทร็กมิสเรจิสเตรชันมีค่าเท่ากับ

![](images/6f47895ddc57e3094051f4ff59ca47383d2c9310f8d8529eae3fca120e03f025.jpg)

<details>
<summary>line</summary>

| nm  | On track (TMR = 0%) | 20% off track (TMR = 20%) | 40% off track (TMR = 40%) |
|-----|---------------------|---------------------------|---------------------------|
| -40 | 0.0                 | 0.0                       | 0.0                       |
| -30 | 0.0                 | 0.0                       | 0.0                       |
| -20 | 0.1                 | 0.1                       | 0.1                       |
...
รูปที่ 6.30 สัญญาณฟัลส์ตามแทรกและสัญญาณฟัลส์ข้ามแทรกที่มีผลกระทบของ TMR

$$
H (m, n) = P (m T _ {z} + \Delta_ {T}, n T _ {x}) \tag {6.21}
$$

ในทางปฏิบัติ TMR มีผลกระทบต่อสมรรถนะของระบบ BPMR ด่อนข้างมาก เพาะมีผลให้สัญญาณอ่านกลับที่ได้มีรูปร่างผิดเพ็ยนไปจากที่ควรจะเป็น รูปที่ 6.30 แสดงสัญญาณพัลล์ตามแทร็ก $^{41}$ (along-track pulse) และสัญญาณพัลล์ช้ามแทร็ก $^{42}$ (cross-track pulse) ที่มีผลกระทบของ TMR โดยอาศัยสมการ (6.14) เมื่อ PW $_{50}^{along}$ = 22.1 nm และ PW $_{50}^{cross}$ = 30 nm และสมมุติว่า $T_{x} = T_{z} = 22$ nm จากรูปพบว่าเมื่อมี TMR เกิดขึ้นในระบบจะส่งผลให้สัญญาณพัลล์ตามแทร็กและสัญญาณพัลล์ช้ามแทร็กมีแอมพลิจุดเปลี่ยนแปลงไปจากเติม ซึ่งทำให้ยากต่อการออตรหัสข้อมูลที่วงจรภาครับ รูปที่ 6.31 แสดงตัวอย่างสมรรถนะของระบบ BPMR ที่เผชิญกับแทร็กมิสรเจิสเตอร์ชันที่มีระดับความรุนแรงต่างๆ จากรูปพบว่าเมื่อแทร็กมิสรเจิสเตอร์ชันมีความรุนแรงมากขึ้นก็จะทำให้ระบบมีสมรรถนะลดลง ดังนี้นระบบ BPMR ควรมีกระบวนการป้องกันและแก่ไขปัญหาเรื่องแทร็กมิสรเจิสเตอร์ชัน เพื่อให้ระบบ BPMR มีสมรรถนะสูงสุด

![](images/1886a5d96c2b304c437c5f44e745f6e8190065c817b59d09df6cd64b145fd385.jpg)

<details>
<summary>line</summary>

| SNR (dB) | On track (TMR = 0%) | 10% off track (TMR = 10%) | 15% off track (TMR = 15%) | 20% off track (TMR = 20%) |
| -------- | ------------------- | -------------------------- | -------------------------- | -------------------------- |
| 10       | 0.01                | 0.01                       | 0.01                       | 0.01                       |
| 11       | 0.005               | 0.005                      | 0.005                      | 0.005                      |
| 12       | 0.002               | 0.002                      | 0.002                      | 0.002                      |
| 13       | 0.001               | 0.001                      | 0.001                      | 0.001                      |
| 14       | 0.0005              | 0.0005                     | 0.0005                     | 0.0005                     |
| 15       | 0.0002              | 0.0002                     | 0.0002                     | 0.0002                     |
| 16       | 0.0001              | 0.0001                     | 0.0001                     | 0.0001                     |
| 17       | 0.00005             | 0.00005                    | 0.00005                    | 0.00005                    |
| 18       | 0.00001             | 0.00001                    | 0.00001                    | 0.00001                    |
</details>

รูปที่ 6.31 ตัวอย่างสมรรถนะของระบบ BPMR ณ ระดับความรุนแรงต่างๆ ของแทร์กมิสเรจิสเตรชั้น (TMR)

# 6.5 สรุปท้ายบท

เทคโนโลยีการบันทึกข้อมูลเชิงแม่เหล็กแบบแนวตั้ง (perpendicular magnetic recording) ซึ่งใช้ในฮาร์ตติสก์ไตรฟ์ปัจจุบัน $^{43}$ กําสังเข้าใกล้ชีดจำกัดชูเปอร์พาราแมนติก ตั้งแสดงในรูปที่ 6.32 ตั้งนั้นเทคโนโลยีอื่นๆ ที่สามารถเพิ่มความหนาแน่นเชิงพื้นที่ (areal density) ได้มากกว่า 1 Tb/in $^{2}$ จึงเป็นสิ่งจำเป็นอย่างยิ่ง ซึ่งได้แก่ เทคโนโลยี BPMR, เทคโนโลยี HAMR, เทคโนโลยี MAMR (microwave assisted magnetic recording), เทคโนโลยี SWR (shingled write recording), และ เทคโนโลยี TDMR โดยแต่ละแบบมีความท้าหายในการออกแบบเพื่อใช้งานจริงตั้งนี้ [117]

1) เทคโนโลยี BPMR ยังคงมีปัญหาเกี่ยวกับการสร้างสื่อบันทึกที่มีความเรียบ (planarization) ในระดับนาโนเมตร, การออกแบบช่องสัญญาณอ่านใหม่ (new read channel), และการเข้าจังหวะการเขียน (write synchronization)   
2) เทคโนโลยี HAMR ยังคงมีปัญหาเกี่ยวกับการสร้างหัวเขียนที่มีเลเซอร์ (laser) เพื่อให้ความร้อน กับสื่อบันทึก, กระบวนการทำให้สื่อบันทึกเย็นตัวลงอย่างรวดเร็ว, และตําแหน่งของการติดตั้งเลเซอร์ที่มีผลกระทบต่อการเขียน   
3) เทคโนโลยี MAMR ยังคงมีปัญหาเกี่ยวกับเครื่องกำเนิดความถี่ของสัญญาณไมโครเวฟ

![](images/5335a98249c5802a2f06790e1777cbea5c471632a08d860f0f98f5c070d32eae.jpg)

<details>
<summary>line</summary>

| Recording Method | Areal density (TDMR) |
| --- | --- |
| 100 Tb/in² | 100 |
| 10 Tb/in² | 10 |
| 5 Tb/in² | 5 |
...
รูปที่ 6.32 แนวโน้มความจุข้อมูลของฮาร์ดติสก์ไตรพ์ [117]

4) เทคโนโลยี SWR ซึ่งใช้ว่วมงานกับเทคโนโลยี TDMR ยังคงมีปัญหาเกี่ยวกับการประมวลผลสัญญาณแบบสองมิติ (2D signal processing) ขึ้นสูงในช่องสัญญาณอ่าน

อย่างไรก็ตามบทนี้ได้นแน่นไปที่เทคโนโลยี BPMR ซึ่งถือว่าเป็นทางเลือกหนึ่งที่ช่วยเพิ่มความจุข้อมูลของฮาร์ตติสก์ไตรฟ์ได้มากกว่าเทคโนโลยีการบันทึกแบบแนวตั้งที่ใช้อยู่ในปัจจุบันโดยบทนี้จะเริ่มต้นการกล่าวถึงงานวิจัยต้านต่างๆ ที่มีผลต่อการพัฒนาเทคโนโลยี BPMR จากนั้นอธิบายวิธีการสร้างผลตอบสนองสัญญาณพัลล์ของระบบ BPMR โดยอาศัยการต่านวณอินทิกรัลการตอบสนองแบบสามมิติเทียบกับศักย์ของหัวอ่านแบบ MR [108] พร้อมทั้งศึกษาผลกระทบของรูปทรงของหัวอ่านและไอแลนด์ที่มีต่อผลตอบสนองสัญญาณพัลล์แบบสองมิตินี้

ในระบบการบันทึกแบบ BPMR สัญญาณอ่านกลับสร้างได้จากการซ้อนทับเชิงเส้นของผลตอบสนองสัญญาณฟ้าลส์ของแทรกหลักและแทรกซ้างเคียง จึงทำให้เกิดปัญหาเรื่องการแทรกสอดระหว่างสัญลักษณ์ (ISI) และการแทรกสอดระหว่างแทรก (ITI) โดยที่ ITI จะมีความรุนแรงมากขึ้น เมื่อความจุข้อมูลสูงขึ้น นอกจากนี้สัญญาณรบกวนหลักที่พบมากในระบบ BPMR ก็คือสัญญาณรบกวนสื่อบันทึกซึ่งเกิดจากความไม่สมบูรณ์ในการผลิตสื่อบันทึก (ทำให้เกิดความผันผวนของตําแหน่ง ขนาด ความสูง และรูปร่าง ของแต่ละไอแลนด์)

ตั้งนั้นบทนี้จึงได้อธิบายแบบจำลองของสัญญาณอ่านกลับในระบบ BPMR ซึ่งใช้หา
ค่าประมาณของสัญญาณพัลส์ BPMR (ที่มีและไม่มีสัญญาณรบกวนสื่อบันทึก) เพื่อให้สามารถ
วิเคราะห์ระบบ BPMR ไดง่ายขึ้น สุดท้ายได้กล่าวถึงปรากฏการณ์แทรกกมิสรเจิสเตอร์น (TMR)
พร้อมทั้งอธิบายการสร้างสัญญาณอ่านกลับที่มีผลกระทบของแทรกกมิสรเจิสเตอร์นต้วย

# 6.6 แบบฝึกหัดท้ายบท

1. จงอธิบายแนวคิดของเทคโนโลยี BPMR   
2. จงอธิบายความหมายของสมการ (6.1)   
3. จงอธิบายสาเหตุและผลกระทบของสัญญาณรบกวนสื่อบั้นทึกที่พบในระบบ BPMR   
4. จงอธิบายสาเหตุและผลกระทบของแทร็กมิสเรจิสเตรชั้น (TMR)   
5. จงใช้โปรแกรม SCILAB วาตรูป

5.1) ผลตอบสนองสัญญาณพัลส์เกาส์เซียนแบบสองมิติตามสมการ (6.14) และ (6.15) โดยให้กำหนดค่าพารามิเตอร์ต่างๆ ด้วยตนเอง พร้อมทั้งทดลองปรับค่าพารามิเตอร์ต่างๆ แล้วสังเกตรูปร่างของผลตอบสนองสัญญาณพัลส์เกาส์เซียนแบบสองมิติตที่ได้   
5.2) สัญญาณฟัลส์ตามแทร็กและสัญญาณฟัลส์ข้ามแทร็กที่มีผลกระทบของ TMR ตามรูปที่ 6.30

6. กำหนดให้ t = 4 nm, W = 20 nm, g = 10 nm, a = 11 nm, δ = 10 nm, และ d = 10 nm จงให้โปรแกรม SCILAB วารูป

6.1) ศักย์แม่เหล็กบนพื้นผิวของหัวอ่านแบบ MR (ตามรูปที่ 6.9)   
6.2) ผลตอบสนองสัญญาณฟัลส์แบบสองมิติ (ตามรูปที่ 6.13)   
6.3) ผลตอบสนองสัญญาณพัลส์ตามแทรกและโพรไฟล์ขวางแทรก (ตามรูปที่ 6.15)

7. ปรับค่าพารามิเตอร์ต่างๆ ที่ใช้ในแบบฝึกหัดท้ายบทที่ข้อ 6 ด้วยตนเอง แล้วทำซ้ําข้อ 6.1 - 6.3 พร้อมหั้งสังเกตและอธิบายผลลัพธ์ที่ได้
