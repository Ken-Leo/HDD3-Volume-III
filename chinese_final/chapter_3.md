...
其中 $\mathbf{y}_0^k = [y_0, y_1, \ldots, y_k]$ 是时间 0 到 k 的待解码序列，$\mathbf{a}_0^k = [a_0, a_1, \ldots, a_k]$ 是对应于 $\mathbf{y}_0^k$ 的输入序列。公式 (3.24) 可重写为：

$$
p(\mathbf{y}_0^k; \mathbf{a}_0^k) = e^{\Phi_{k+1}(q)} \tag{3.25}
$$

SOVA 算法输出的数据位判决可靠性（对于二进制码）可按如下方式求得。从图 3.7 可以看出，有两条状态转移路径到达时间 k+1 的状态 q，即 (u, q) 和 (s, q)，其路径度量分别为 $\Phi_{k+1}^{(1)}(q)$ 和 $\Phi_{k+1}^{(2)}(q)$。假设 $\Phi_{k+1}^{(1)}(q) > \Phi_{k+1}^{(2)}(q)$，则路径 (1) 是到达状态 q 的最佳路径，SOVA 选择路径 (1) 作为到达状态 q 的幸存路径 $\mathbf{S}_{k+1}(q)$。
