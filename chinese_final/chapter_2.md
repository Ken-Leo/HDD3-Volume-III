...
Turbo 码被视为一种高性能的纠错码 (ECC)，复杂度不高，并被用作第三代移动通信系统 (3G) 的通信标准。Turbo 编码使用多个卷积编码器以串联或并联方式连接，借助交织器的帮助。而 Turbo 解码则使用多个 BCJR 解码器（数量与所使用的编码器数相等），彼此交换软信息。此外，Turbo 码的数据解码是一种迭代解码，可应用于均衡过程，称为 Turbo 均衡，已被应用于新一代硬盘驱动器中。

因此，本章介绍了迭代解码技术的组成部分和工作原理，从卷积编码和解码、Turbo 解码器中使用的 BCJR 算法、Turbo 编码和解码，到 Turbo 均衡，同时展示了 Turbo 码和 Turbo 均衡器的性能。实验结果表明，使用迭代解码的系统性能优于不使用迭代解码技术的系统。此外，一般来说，使用迭代解码的系统随着迭代次数的增加，性能会提高（当系统工作在足够高的 SNR 时）。

# 2.6 习题

1. 请绘制图 2.1(b) 和 2.1(c) 中 Turbo 编码器的有限状态机 (FSM) 和网格图。
2. 针对图 2.1 中的三种 Turbo 编码器，对以下数据序列进行编码，求输出序列 $y_k^1$ 和 $y_k^2$：
   2.1) $x_k = \{1\;1\;0\;1\}$
   2.2) $x_k = \{1\;0\;1\;0\;1\}$
   2.3) $x_k = \{1\;1\;0\;1\;0\;1\}$
3. 从示例 2.2 出发，使用图 2.6 中的卷积编码器对以下数据序列进行编码：
   3.1) $\{1011\}$
   3.2) $\{110101\}$
   3.3) $\{10110011\}$
4. 从示例 2.3 出发，对以下数据序列进行解码：
   4.1) $z_k = \{10\;01\;11\;01\}$
   4.2) $z_k = \{11\;10\;00\;10\;01\}$
   4.3) $z_k = \{01\;00\;10\;11\;00\;10\}$
5. 请证明方程 (2.26)。
6. 考虑图 2.10 中的信道模型。设输入数据序列 $a_k = \{1, 1, -1\}$，信道 $H(D) = 1 - 0.5D$，噪声 $n_k = \{-0.2, -0.1, 0.2, 0.1\}$，方差 $\sigma^2 = 1/(2\pi)$。使用 BCJR 算法解码数据序列 $y_k$，设数据位 $a_k$ 的先验信息为 $\lambda_a(a_k) = \{0.2, 0.1, -0.2, 0\}$。
7. 考虑图 2.10 中的信道模型。设输入数据序列 $a_k = \{1, 1, -1\}$，信道 $H(D) = 1 + 2D + D^2$，噪声 $n_k = \{-0.2, -0.3, 0.2, -0.4, 0.3\}$。使用 BCJR 算法解码数据序列 $y_k$，设数据位 $a_k$ 的先验信息为 $\lambda_a(a_k) = \{0.2, 0.1, -0.2, 0, 0\}$。
8. 使用图 2.18 中的 Turbo 编码器对数据序列 $\{x_0\;x_1\;x_2\;x_3\} = \{1011\}$ 进行编码。设所使用的交织器输出为 $\{x_2\;x_0\;x_3\;x_1\}$，两个子编码器的生成多项式为：
   8.1) $(g_1, g_2) = (07, 05)$
   8.2) $(g_1, g_2) = (17, 11)$
   8.3) $(g_1, g_2) = (23, 37)$
   8.4) $(g_1, g_2) = (37, 21)$
...
9. 请解释交织器的工作原理和重要性。
10. 请解释外信息 (extrinsic information) 的含义。
11. 请解释并行级联 Turbo 码和串行级联 Turbo 码的区别。
12. 请解释删余器 (puncturer) 的工作原理，并举例说明计算过程。
13. 请解释 Turbo 均衡的工作原理。
