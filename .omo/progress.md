# Translation Progress

Overall Status: PDF HDD3-Volume III → Chinese translation
Goal: Full book translation (Ch1-8 + Appendixes)

## Page Mapping
Offset: Physical = Logical + 13 (Physical P14 = Logical p1)

| Chapter | Physical Pages | Logical Pages |
|---------|---------------|---------------|
| Ch1 概述 | P14-P28 | p1-p15 |
| Ch2 Turbo均衡 | P30-P76 | p17-p63 |
| Ch3 软输出算法 | P78-P118 | p65-p105 |
| Ch4 LDPC码 | P120-P167 | p107-p154 |
| Ch5 迭代解码应用 | P168-P207 | p155-p194 |
| Ch6 BPMR技术 | P208-P241 | p195-p228 |
| Ch7 BPMR系统设计 | P242-P275 | p229-p262 |
| Ch8 HAMR | P276-P319 | p263-p306 |
| 附录A | P320-P321 | p307-p308 |
| 附录B | P322 | p309 |
| 附录C | P324 | p311 |
| 附录D | P326 | p313 |

## Extraction Status
- [x] Ch1: 318行 / 27图 / 24公式 ✓
- [x] Ch2: 955行 / 45图 / 88公式 ✓
- [x] Ch3: 1023行 / 13图 / 114公式 ✓
- [x] Ch4: 1147行 / 30图 / 133公式 ✓
- [x] Ch5: 612行 / 35图 / 46公式 ✓
- [x] Ch6: 498行 / 46图 / 25公式 ✓
- [x] Ch7: 998行 / 53图 / 100公式 ✓
- [x] Ch8: 572行 / 29图（§8.1-§8.7.1完整，mineru重新提取完成）✓
- [ ] 附录A-D: 未提取（PDF P320-P326，主要含图）
- [x] 参考文献: 547行（appendix.md，[30]-[144]）

## Translation Status
- [x] Ch1: 完成（316行）
- [x] Ch2: 完成（930行，含习题5）
- [x] Ch3: 完成（832行，习题完整）
- [x] Ch4: 完成（963行）
- [x] Ch5: 完成（363行）
- [x] Ch6: 完成（316行，含§6.5-§6.6+注释）
- [x] Ch7: 完成（689行，标题已修复）
- [ ] Ch8: 259/564行翻译完成（仅§8.1-§8.4.3，§8.4.4-§8.7.1待翻译）⚠
- [x] 参考文献: 完成（546行，无需翻译）

## Translation Log
| Date | Chapter | Lines | Content |
|------|---------|-------|---------|
| 2026-07-01 | Ch1 | 1-317 | 全部完成 |
| 2026-07-02 | Ch2 | 1-927 | 全部完成 |
| 2026-07-03 | Ch6 | 316 | 追加§6.5-§6.6+注释 |
| 2026-07-03 | Ch2 | 930 | 追加习题5 |
| 2026-07-03 | Ch7 | 689 | 修复标题为"第7章 BPMR系统的目标和均衡器设计" |
| 2026-07-03 | Ch8 | 259 | 从Ch7源中提取并翻译 |
| 2026-07-03 | Ch8 | 564 | 重新用mineru提取，新增§8.4.4-§8.7.1内容（27张新图） |

## Known Issues
1. **Ch8翻译不完整**: 源文件已重新提取（564行/§8.1-§8.7.1），但中文仅翻译了前259行（§8.1-§8.4.3），§8.4.4-§8.7.1（纵向HAMR的dH_d/dx、x0/a求解、垂直HAMR、微磁道模型、系统特性）以及Ch8习题待翻译
2. **附录A-D未提取**: PDF P320-P326主要含图，需mineru重新提取
3. **Ch3/Ch4/Ch5/Ch7翻译完整性**: 中文字数显著少于源文件，需逐章核对
